...
(defun ai-move-easy(&optional col-list)
  ...
  (let ((col (pick-random-from-list col-list)) (row 1) (player 0) (flag 0))   ; Set local variables. Call 'pick-random-from-list' to get the random column number.
    (loop for move in game do                                                 ; Loop through each move in the game global variable.
      (if (and (equal (nth 0 move) col) (>= (nth 1 move) 6))                  ; If the column in the game list move == the random column move chosen and it's row >= 6...
        ...
        (if (equal (nth 0 move) col)                                          ; Otherwise, if the column in the game move == the random column move, and row < 6...
...
(defun ai-move-intm()
  ...
  (let ((concurrent-moves '()) (canidate-move nil))
    ...
    (loop for i from 0 to (- (length move-positions) 1) by 7 do

      (loop for n from 0 to 6 do
        ; Check horizontal to the right
        (if (and (eq (nth (+ i n) move-positions) 0) (not (eq n 6)))
        ...
        (if (eq (nth (+ i n) move-positions) 0)
          (if (eq (nth (+ i n 7) move-positions) -1)
            (setf concurrent-moves (append concurrent-moves (list (+ i n 7))))))

        ; Check diagonal right
        (if (and (not (eq n 6)) (> i 6))
          (if (eq (nth (+ i n) move-positions) 0)
            (if (eq (nth (+ i n 8) move-positions) -1)

              ; Make sure there's a piece under this one...
              (if (eq T (check-below (+ i n 8)))
                (setf concurrent-moves (append concurrent-moves (list (+ i n 8))))))))

        ; Check diagonal left
        (if (and (not (eq n 0)) (> i 6))
          (if (eq (nth (+ i n) move-positions) 0)
