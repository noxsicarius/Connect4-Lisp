...
;;;
;;; @author Jason Pollman, Cullin Moran, Walter Alfaro, Robert Iannucci
;;; 11/12/13
...
(defun check-moves()
  ...
  (let ((message "~%") (i 1))
    ...
    (if (>= (length game) 42)
      (setf message (concatenate 'string message "The number of total moves == 42. On an 7x6 board any more than this would be illegal. The game should be over by now.~%")))
    
    (loop for move in game do
      (cond
        ;; Check that a column move is <= 7
        ((or (> (nth 0 move) 7) (< (nth 0 move) 1)) (setf message (concatenate 'string message "Move #" (write-to-string i) " contained a column with a value > 7.~%")))
        ;; Check that a row move is <= 6
        ((or (> (nth 1 move) 6) (< (nth 1 move) 1)) (setf message (concatenate 'string message "Move #" (write-to-string i) " contained a row with a value > 6.~%")))
